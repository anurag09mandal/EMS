// ============================================================================
// File: logcollector_application.cpp
// Description: Implementation of logcollector_application class handling custom logic
// Author: 
// Created: 2025-11-20
// Updated:
// License: Internal use only 
// ============================================================================

#include "logcollector_application.hpp"
#include <iostream>
#include <cstdlib>
#include <ctime>
#include <random>
#include "edge_logger.hpp"
#include "logger_utils.hpp"
#include "metric_loader.hpp"
#include <unistd.h>

struct LogTask {
    std::string metricName;
    std::string value;
    std::string fileName; 
};


static std::unordered_map<std::string, std::queue<LogTask>> deviceQueues;
static std::unordered_map<std::string, std::mutex> deviceMutexes;
static std::unordered_map<std::string, std::condition_variable> deviceCVs;

// Store jthreads for cleanup
static std::unordered_map<std::string, std::jthread> deviceThreads;
 
static std::unordered_map<std::string, std::vector<std::string>> deviceCategoryMap = {
    {"BMS", {"BO","BF","BM","BLC"}},
    {"PCS", {"PCCM","PCF","PCFM"}},
    {"PMU", {"PMM","PMF"}},
    {"MI",  {"ME","MM","MDI"}},
    {"EMS", {"EM","EI"}},
    {"AHM", {"AHM"}}
};


// Constructor: Initializes the LogCollector with a given client ID
LogCollector::LogCollector(const std::string& client_id) : BaseApp(client_id) {
    EdgeLogger::get().set_app_name_mapping(client_id, "TA");
    std::srand(std::time(nullptr));
}

LogCollector::~LogCollector() {
}

void LogCollector::cleanup() {
    LOG(INFO) << "LogCollector cleanup" << std::endl;
    cleanup_called();

    if (message_sender_thread_.joinable()) {
        message_sender_thread_.request_stop();
    }
    // Stop all category threads
    for (auto& [deviceType, thread] : deviceThreads) {
        if (thread.joinable()) thread.request_stop();
    }
}


void deviceWorker(std::stop_token stoken, const std::string& deviceType) {
    while (!stoken.stop_requested()) {
        std::unique_lock<std::mutex> lock(deviceMutexes[deviceType]);
        deviceCVs[deviceType].wait(lock, [&] {
            return !deviceQueues[deviceType].empty() || stoken.stop_requested();
        });

        if (stoken.stop_requested()) break;

        auto task = deviceQueues[deviceType].front();
        deviceQueues[deviceType].pop();
        lock.unlock();

        std::cout << "[Thread " << deviceType << "] Logging: "
                  << task.metricName << " = " << task.value << std::endl;

        LoggerUtils::instance().logMetricToCSV(task.metricName, task.value, task.fileName);
    }

    std::cout << "[Thread " << deviceType << "] Stopping gracefully." << std::endl;
}



void initDeviceThreads() {
    for (const auto& [deviceType, categories] : deviceCategoryMap) {
        deviceThreads[deviceType] = std::jthread(deviceWorker, deviceType);
    }
}



void LogCollector::start() {
    initialize_settings();
    notify_app_ready();
    LoggerUtils::instance().initLogMapping();
    initializeMetricLoading();
    initDeviceThreads();
    message_sender_thread_ = std::jthread(&LogCollector::message_sender_loop, this);

}

void LogCollector::message_sender_loop(std::stop_token stop_token) 
{
    std::cout << "Message sender loop started v1.0" << std::endl;
    while(!stop_token.stop_requested())
    {   
        for (const auto& req : requestMetrics) {
            nlohmann::json payload;
            payload["metricPath"] = req.metricPath;
            publish_request(APPID_ENERGY_LINK, SERVICE_SEND_READ_METRIC, payload);
            std::this_thread::sleep_for(std::chrono::milliseconds(1000));
        }
    }
}


void handle_on_demand_read_response(const nlohmann::json& message) {
    std::string metricPath = message["metricPath"];
    std::string value = message["value"].dump();
    std::string metricName = metricPath.substr(metricPath.find_last_of('/') + 1);

    auto& logMapping = LoggerUtils::instance().getLogMapping();
    auto it = logMapping.find(metricName);
    if (it == logMapping.end()) {
        std::cerr << "[OnDemandResponse] Unknown metric: " << metricName << std::endl;
        return;
    }


    std::string category = it->second.logType; // e.g., BO
    std::string deviceType;
    for (const auto& [dev, cats] : deviceCategoryMap) {
        if (std::find(cats.begin(), cats.end(), category) != cats.end()) {
            deviceType = dev;
            break;
        }
    }

    {
        std::lock_guard<std::mutex> lock(deviceMutexes[deviceType]);
         deviceQueues[deviceType].push({metricName, value, it->second.logFileName});
    }
    deviceCVs[deviceType].notify_one();

}



void LogCollector::handle_request(const std::string& tid, const std::string& timestamp, const std::string& source, const std::string& service, const nlohmann::json& message) {
    if(service == SERVICE_SHUTDOWN_APPLICATION || service == SERVICE_SHUTDOWN_APPLICATION_FOR_SYSTEM_REBOOT)
    {
        cleanup();
        nlohmann::json res_payload;
        publish_response(tid,res_payload);
    }
}


void LogCollector::handle_response(const std::string& tid, const std::string& timestamp, const std::string& source, const std::string& service, const nlohmann::json& message) {
   if(service == SERVICE_SEND_READ_METRIC) //SERVICE_ON_DEMAND_READ)
   {
      std::cout << "Handle on demand response" << std::endl;

      handle_on_demand_read_response(message);

   } 
}

void LogCollector::handle_notification(const std::string& timestamp, const std::string& source, const std::string& service, const nlohmann::json& message) {
LOG(DEBUG) << "[test] IPC notification " << source << " "  << service << " : " << message.dump(4) << std::endl;
   if(service == NOTI_TELEMETRY)
   {
       LOG(DEBUG) << "Noti Serrvice Telemetry: " << message["message"].get<std::string>() << std::endl;
   }
   if(service == NOTI_CHANGED_DEVICE_CONNECTION)
   {

   }
}

void LogCollector::handle_noti_system_settings_changed(std::vector<std::string> keys) {
  
}

void LogCollector::handle_noti_persistent_states_changed(std::vector<std::string> keys) {

}

void LogCollector::handle_message(const std::string& topic, const nlohmann::json& message) {
	LOG(DEBUG) << "Callback for handling messages" << "topic : " << topic << message["message"].get<std::string>() << std::endl;
}





